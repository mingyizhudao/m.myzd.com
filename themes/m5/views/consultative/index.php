    ...
    <div class="wifi-contain">
        ...
        <div class="mid-contain">
            ...
            <div class="w-b-2">
                ...
                <img class="w-b-2" src="http://static.mingyizhudao.com/147808217204330">
                <ul>
...
<script>
    $(document).ready(function(){$(".freePhone").click(function(){J.customConfirm("友情提示",'<div class="mb10">立即拨打免费客服热线400-6277-120</div>','<a id="closeLogout" class="w50">取消</a>','<a id="dial" class="color-green w50">拨打</a>',function(){});$("#closeLogout").click(function(){J.closePopup()});$("#dial").click(function(){J.closePopup();location.href="tel://4006277120"})});$(".online-adv").click(function(){window.open("http://dct.zoosnet.net/LR/Chatpre.aspx?id=DCT73779034&lng=cn","_blank")});$("#btn-sendSmsCode").click(function(e){e.preventDefault();checkCaptchaCode($(this))});$("#btn-booking").click(function(e){e.preventDefault();commitBookingInfo()})});function checkCaptchaCode(domBtn){var domMobile=$("#booking_mobile");var mobile=domMobile.val();if(mobile.length===0){J.showToast("手机号不得为空","","1500");return false}if(!(/^1[34578]\d{9}$/.test(mobile))){J.showToast("手机号码有误，请重填","","1500");return false}var smsParams={smsVerifyCode:{mobile:mobile,action_type:200}};$.ajax({type:"post",url:"<?php echo $urlGetSmsVerifyCode ?>",data:smsParams,success:function(data){if(data.status){J.showToast("验证码已发送","","1500");$(domBtn).prop("disabled",true);$(domBtn).html("已发送");setTimeout(function(){$("#btn-sendSmsCode").prop("disabled",false);$("#btn-sendSmsCode").html("验证码")},60000);return true}else{console.log("err",data);J.showToast("获取验证码失败，请重试","","1500");return false}}})}function commitBookingInfo(){var domMobile=$("#booking_mobile");var mobile=domMobile.val();if(mobile.length===0){J.showToast("手机号不得为空","","1500");return false}if(!(/^1[34578]\d{9}$/.test(mobile))){J.showToast("手机号码有误，请重填","","1500");return false}var domVerify=$("#booking_code");var verify=domVerify.val();if(verify.length===0){J.showToast("验证码不得为空","","1500");return false}var domName=$("#booking_name");var name=domName.val();if(name.length===0){J.showToast("姓名不得为空","","1500");return false}if(name.length>10){J.showToast("姓名长度不得超过10个字符","","1500");return false}var docName=$("#booking_doctor");var doctor=docName.val();var formdata={booking:{mobile:mobile,verify_code:verify,contact_name:name,doctor_name:doctor}};$.ajax({type:"post",url:"<?php echo $urlGetBooking ?>",data:formdata,success:function(data){if(data.status=="ok"){J.showToast("预约成功！我们的客服人员会尽早联系你！","","1500")}else{J.showToast(data.errorMsg,"","1500")}}})};
</script>