...
class OrderController extends MobileController
{
    ...
    public function actionPayDeposit()
    {
        ...
        if ($_POST['order']['bk_ref_no']) {
            ...
            $model = SalesOrder::model()->getByAttributes(array(
                'bk_ref_no' => $refno,
                'order_type' => 'deposit'
            ));
            if ($model) {
                ...
                if ($booking->booking_service_id == BookingServiceConfig::BOOKING_SERVICE_FREE_LIINIC) {
                    $adminDate = AdminBooking::model()->updateAllByAttributes(array(
                        'booking_status' => StatCode::BK_STATUS_PROCESSING,
                        'work_schedule' => StatCode::BK_STATUS_PROCESSING,
                        'date_updated' => new CDbExpression("NOW()")
                    ), array(
                        'ref_no' => $refno
                    ));
                    $orderDate = SalesOrder::model()->updateAllByAttributes(array(
                        'is_paid' => SalesOrder::ORDER_PAIDED,
                        'date_closed' => new CDbExpression("NOW()")
                    ), array(
                        'bk_ref_no' => $refno,
                        'order_type' => 'deposit'
                    ));
                    $bookingDate = Booking::model()->updateAllByAttributes(array(
                        'bk_status' => StatCode::BK_STATUS_PROCESSING
                    ), array(
                        'ref_no' => $refno
                    ));
