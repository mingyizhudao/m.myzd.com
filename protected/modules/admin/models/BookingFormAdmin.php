...
class BookingFormAdmin extends EFormModel {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('patient_condition, remark', 'length', 'max' => 500, 'min' => 10, 'tooShort' => '至少输入10个字'),
            array('contact_email, contact_weixin', 'length', 'max' => 100),
    ...
    public function initModel($userId, Booking $model = null) {
        if (isset($model)) {
            ...
            $this->_booking_type_text = $model->getBookingTypeText();
            $this->_status_text = $model->getStatusText();
            $this->_options_status = $model->getOptionsStatus();
        } else {
            $this->user_id = $userId;
            $this->status = Booking::STATUS_NEW;
        }
        $this->_setScenarioAsBookingType();
    }
    ...
    private function _setScenarioAsBookingType() {
        if (is_null($this->booking_type) === false) {
            $this->scenario = $this->booking_type;
        }
    }
    ...
    public function getBookingTypeText() {
        return $this->_booking_type_text;
    }
