 ...
 *
 * The followings are the available columns in table 'feed_back':
 * @property integer $id
...
class FeedBack extends EActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('user_host_ip, source, contact_mobile', 'length', 'max'=>20),
			array('content', 'length', 'max'=>1000),
			array('date_updated, date_deleted', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, user_host_ip, source, contact_mobile, content, date_created, date_updated, date_deleted', 'safe', 'on'=>'search'),
		);
	}
