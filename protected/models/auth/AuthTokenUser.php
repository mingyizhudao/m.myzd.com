...
class AuthTokenUser extends EActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array(
                'id, token, user_id, username, is_active, time_expiry, user_host_ip, user_mac_address, date_created, date_updated, date_deleted',
                'safe',
                'on' => 'search'
            )
        );
    ...
    public function createWapToken($userId, $username, $userRole, $userHostIp, $userMacAddress)
    {
        $this->setUserId($userId);
        $this->setUsername($username);
        $this->setUserRole($userRole);
    ...
    // 验证 token。
    public function verifyTokenPatient($token, $username, $agent = NULL)
    {
        return $this->verifyByTokenAndUsernameAndRole($token, $username, StatCode::USER_ROLE_PATIENT, $agent);
    }
...
//             exit;
    ...
    public function getWapTimeExpiry()
    {
        return $this->time_expiry;
    }
